...
function CompareProductCard({
  ...
  return (
    <article
      onClick={onSelect}
      className="w-[148px] sm:w-[168px] shrink-0 snap-start welcome-card-glow welcome-card-border-glow bg-white/5 border border-white/30 backdrop-blur-md rounded-2xl shadow-lg hover:bg-white/10 transition-all duration-300 cursor-pointer group overflow-hidden flex flex-col"
    >
      <div className="relative aspect-square bg-white/10 p-2">
        <ProductImage
          src={product.image}
          alt={product.name}
          size="custom"
        ...
        />
        <span
          className={`absolute bottom-2 left-2 right-2 inline-flex items-center justify-center gap-1 px-2 py-1 rounded-lg border text-[8px] font-black backdrop-blur-sm ${badgeClass}`}
        >
          <BadgeIcon size={10} />
      ...
      <div className="p-2.5 flex flex-col flex-1 text-right min-h-[118px] bg-gradient-to-r from-[#7B3DFF]/90 to-[#0B1320] border-t border-white/15">
        ...
        <div className="mt-2 pt-2 border-t border-white/15 flex items-end justify-between gap-1">
          ...
          <button
            ...
            className="welcome-btn-pulse relative z-10 w-7 h-7 text-white border border-white/40 bg-white/10 hover:bg-[#fff700] hover:text-deep-navy rounded-lg flex items-center justify-center transition-colors shrink-0 active:scale-95 shadow-sm"
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between gap-2 px-0.5">
        ...
        <span className="text-[10px] font-bold text-white/55 bg-white/10 border border-white/20 px-2 py-0.5 rounded-full">{items.length} منتج</span>
      </div>
...
export const ProductComparePanel: React.FC<ProductComparePanelProps> = ({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 40 }}
      className="fixed inset-0 bg-deep-navy/80 backdrop-blur-xl z-[90] flex items-end sm:items-center justify-center p-0 sm:p-4"
    >
      <div
        className="relative welcome-card-glow welcome-card-border-glow bg-deep-navy w-full max-w-3xl h-[100dvh] sm:h-auto sm:max-h-[92vh] sm:rounded-[2rem] shadow-2xl overflow-hidden flex flex-col text-right border border-white/30"
        dir="rtl"
      >
        <WelcomeScreenBackground lite />
        <div className="relative z-10 flex flex-col h-full sm:max-h-[92vh] overflow-hidden">
          <div className="welcome-card-shimmer shrink-0 px-4 py-3 flex items-center justify-between gap-2 border-b border-white/15 bg-white/5 backdrop-blur-md pt-[max(0.75rem,env(safe-area-inset-top))]">
            <button
              type="button"
              onClick={onBack ?? onClose}
              className="welcome-btn-pulse flex items-center gap-1.5 text-white bg-white/15 hover:bg-white/25 border border-white/25 rounded-xl px-3 py-2 text-[11px] font-black transition-all active:scale-95 shrink-0"
            >
          ...
          <div className="shrink-0 p-4 border-b border-white/15 bg-white/5 backdrop-blur-md">
            <div className="flex items-center gap-3">
              <div className="w-16 h-16 rounded-2xl overflow-hidden bg-white/10 border border-white/20 shrink-0">
                <ProductImage
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6">
            {comparisons.length === 0 ? (
              <div className="py-16 text-center rounded-[2rem] border border-white/20 bg-white/5 px-6 welcome-card-border-glow">
