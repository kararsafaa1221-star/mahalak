...
const customerPublicDir = path.resolve(__dirname, 'public');
...
function customerPublicAssetsPlugin(): Plugin {
  return {
    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        if (!rel) return next();
        const filePath = path.join(customerPublicDir, rel);
    ...
    closeBundle() {
      if (!fs.existsSync(customerPublicDir)) return;
      for (const name of fs.readdirSync(customerPublicDir)) {
        const src = path.join(customerPublicDir, name);
...
export default defineConfig({
  ...
  envDir: __dirname,
  envPrefix: 'VITE_',
  plugins: [react(), tailwindcss(), customerPublicAssetsPlugin()],
  resolve: {
    alias: {
  ...
  define: {
    global: 'window',
    'import.meta.env.VITE_MAHALAK_APP': JSON.stringify('customer'),
  },
  base: './',
