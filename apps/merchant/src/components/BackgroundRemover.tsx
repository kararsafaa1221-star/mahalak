...
import { Camera, X, Image as ImageIcon, Sparkles, RefreshCw, Check, Upload, Palette, ShieldAlert, Move, ZoomIn, ZoomOut, Crosshair } from 'lucide-react';
import {
  extractFocusedRegion,
  getProgressLabel,
  removeBackgroundOptimized,
  resetBgRemovalPreload,
} from '@shared/utils/backgroundRemoval';
import { computeCoverDimensions } from '@shared/utils/imageCrop';
...
export const BackgroundRemover: React.FC<BackgroundRemoverProps> = ({
  ...
  const [customColor, setCustomColor] = useState<string>('#7B3DFF');
  ...
  // Preview-step focus framing (before AI processing)
  const [previewZoom, setPreviewZoom] = useState<number>(1.15);
  ...
  const previewDragStart = useRef<{ x: number; y: number }>({ x: 0, y: 0 });
  const [previewImageDimensions, setPreviewImageDimensions] = useState({ width: 0, height: 0 });
  const [previewContainerSize, setPreviewContainerSize] = useState(320);
  const [resultImageDimensions, setResultImageDimensions] = useState({ width: 0, height: 0 });
  const [resultContainerSize, setResultContainerSize] = useState(320);

  // Result-step positioning (after AI processing)
  ...
  const handlePreviewPointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    previewDragStart.current = { x: e.clientX - previewPanX, y: e.clientY - previewPanY };
    e.currentTarget.setPointerCapture(e.pointerId);
  };

  const handlePreviewPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    if (!previewDragging) return;
    setPreviewPanX(e.clientX - previewDragStart.current.x);
    setPreviewPanY(e.clientY - previewDragStart.current.y);
  };
  ...
  // Preload AI model as soon as the modal opens so processing starts faster.
  useEffect(() => {
    if (!isOpen) return;
    resetBgRemovalPreload();
  }, [isOpen]);

  // Read image dimensions when a new capture is selected.
  useEffect(() => {
    if (!capturedImage) {
  ...
  // Track preview container size for accurate crop math.
  useEffect(() => {
    if (step !== 'preview' || !previewContainerRef.current) return;
    const el = previewContainerRef.current;
    const update = () => setPreviewContainerSize(el.clientWidth || 320);
    update();
  ...
  const previewRenderSize = useMemo(
    ...
    [previewImageDimensions.width, previewImageDimensions.height, previewContainerSize],
  );

  useEffect(() => {
    if (!processedUrl) {
      setResultImageDimensions({ width: 0, height: 0 });
      return;
    }
    const img = new window.Image();
    img.onload = () => {
      setResultImageDimensions({ width: img.width, height: img.height });
      setZoom(1);
      setPanX(0);
      setPanY(0);
    };
  ...
  useEffect(() => {
    if (step !== 'result' || !containerRef.current) return;
    const el = containerRef.current;
    const update = () => setResultContainerSize(el.clientWidth || 320);
    update();
    const observer = new ResizeObserver(update);
    observer.observe(el);
    return () => observer.disconnect();
  ...
  const resultRenderSize = useMemo(
    () => computeCoverDimensions(
      resultImageDimensions.width,
      resultImageDimensions.height,
      resultContainerSize,
      resultContainerSize,
    ),
    [resultImageDimensions.width, resultImageDimensions.height, resultContainerSize],
  );
  ...
  const runBackgroundRemoval = async () => {
    ...
    setErrorText(null);
    setProgressPct(0);
    setProgressText('جاري تحضير الصورة وتركيز المنتج...');

    try {
      const containerSize = previewContainerRef.current?.clientWidth || 320;
      const focusedBlob = await extractFocusedRegion(capturedImage, {
        panX: previewPanX,
        panY: previewPanY,
        zoom: previewZoom,
        containerSize,
      ...
      setProgressText('جاري تحميل نموذج الذكاء الاصطناعي...');
      resetBgRemovalPreload();
      const outputBlob = await removeBackgroundOptimized(focusedBlob, (key, pct) => {
        setProgressPct(pct);
        setProgressText(`${getProgressLabel(key)}: ${pct}%`);
      });
  ...
  return (
    <div className="fixed inset-0 bg-deep-navy/50 backdrop-blur-md z-[200] flex items-center justify-center p-4" dir="rtl">
      <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden border border-slate-100 flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-4">
            ...
            <div className="space-y-4 flex flex-col items-center">
              ...
              {hasPermission === true && (
                <div className="relative w-full max-w-sm rounded-3xl overflow-hidden shadow-lg border-4 border-slate-100 aspect-square">
                  ...
                  />
                  <div className="absolute inset-0 border-2 border-vibrant-purple m-6 rounded-2xl opacity-40 border-dashed pointer-events-none"></div>
                </div>
              )}
          ...
          {/* STEP 3: Preview & focus frame before AI processing */}
          {step === 'preview' && capturedImage && (
            <div className="space-y-4 flex flex-col items-center">
              <div className="flex items-center gap-2 text-center">
                <Crosshair size={14} className="text-violet-600 shrink-0" />
                <h4 className="text-xs font-black text-slate-700">
                  ركّز المنتج داخل الإطار قبل التفريغ
                </h4>
              </div>
              <p className="text-[10px] text-slate-500 font-bold text-center leading-relaxed max-w-xs">
                كبّر وحرّك الصورة حتى يملأ المنتج المربع — كلما كان أوضح داخل الإطار، كانت النتيجة أسرع وأدق
              </p>
              ...
              <div
                ...
                className="relative w-full max-w-xs rounded-2xl overflow-hidden border-2 border-violet-300 aspect-square shadow-md bg-slate-100 select-none touch-none"
              ...
              >
                ...
                />
                <div className="absolute inset-4 border-2 border-dashed border-vibrant-purple rounded-xl pointer-events-none opacity-70" />
                <div className="absolute top-2 left-1/2 -translate-x-1/2 bg-violet-600/90 text-white text-[9px] font-black px-2.5 py-1 rounded-full pointer-events-none">
                  منطقة التركيز
                </div>
              </div>
              ...
              <div className="w-full max-w-xs bg-slate-50 px-4 py-3 rounded-2xl border border-slate-100 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black text-slate-500 flex items-center gap-1">
                    <Move size={12} className="text-violet-500" />
                    <span>تكبير وموضع المنتج:</span>
                  </span>
                  <button
                    type="button"
                    onClick={() => {
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => setPreviewZoom((prev) => Math.min(3.0, prev + 0.1))}
                    className="p-1.5 bg-white border border-slate-200 rounded-lg hover:bg-slate-100 transition text-slate-600 shadow-sm"
                  >
                    <ZoomIn size={14} />
                  </button>
              ...
              <div className="grid grid-cols-2 gap-3 w-full max-w-sm mt-1">
                <button
                  ...
                  className="p-4 bg-vibrant-purple hover:bg-[#823ce6] text-white border border-violet-400 rounded-2xl flex flex-col items-center text-center gap-2 transition group shadow-md"
                >
          ...
          {step === 'processing' && (
            <div className="py-12 flex flex-col items-center justify-center text-center space-y-6">
              ...
              <div className="space-y-2 max-w-sm w-full px-4">
                ...
                <div className="p-3 bg-amber-50 rounded-2xl border border-amber-100">
                  <p className="text-[9px] text-amber-700 font-bold leading-normal">
                    ⏳ المرة الأولى قد تستغرق دقيقة لتحميل النموذج. بعدها تصبح المعالجة أسرع بكثير.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col items-center space-y-3">
                <div 
                  ...
                  className="relative w-full max-w-xs rounded-3xl overflow-hidden shadow-inner aspect-square border-2 border-slate-200 select-none touch-none"
                  style={{
                ...
                >
                  <img 
                    ...
                    style={{
                      left: '50%',
                      top: '50%',
                      width: `${resultRenderSize.width}px`,
                      height: `${resultRenderSize.height}px`,
              ...
              <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <button
                    ...
                    className={`py-2 px-3 border rounded-xl text-center text-xs font-bold transition flex items-center justify-center gap-1.5 ${bgType === 'transparent' ? 'bg-vibrant-purple text-white border-transparent' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-100'}`}
                  >
                    <span>شفاف 🏁</span>
                  </button>
                  ...
                  <button
                    ...
                    className={`py-2 px-3 border rounded-xl text-center text-xs font-bold transition flex items-center justify-center gap-1.5 ${bgType === 'white' ? 'bg-vibrant-purple text-white border-transparent' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-100'}`}
                  >
                    <span>أبيض ناصع ⬜</span>
                  </button>
                  ...
                  <button
                    ...
                    className={`py-2 px-3 border rounded-xl text-center text-xs font-bold transition flex items-center justify-center gap-1.5 ${bgType === 'custom' ? 'bg-vibrant-purple text-white border-transparent' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-100'}`}
                  >
                    <Palette size={14} />
                    <span>لون مخصص 🎨</span>
                  </button>
