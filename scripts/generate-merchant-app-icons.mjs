#!/usr/bin/env node
/**
 * Generates web + Android + iOS launcher icons and splash screens for the merchant app.
 * Run: node scripts/generate-merchant-app-icons.mjs
 */
...
const src = join(root, 'apps', 'merchant', 'resources', 'icon.png');
const webIcon = join(root, 'apps', 'merchant', 'public', 'icon.png');
const androidRes = join(root, 'apps', 'merchant', 'android', 'app', 'src', 'main', 'res');
const iosIcon = join(
  root,
  'apps',
  'merchant',
...
const BACKGROUND = { r: 26, g: 10, b: 32, alpha: 1 }; // #1A0A20
...
async function main() {
  ...
  console.log('Generating merchant app icons…');

  mkdirSync(dirname(webIcon), { recursive: true });
  copyFileSync(src, webIcon);
  console.log('  ✓ apps/merchant/public/icon.png');
